 ...
 * @LastEditTime: 2020-01-07 11:03:51
...
import 'package:qq/ui/popMenu/popup_menu_route_layout.dart';
...
class _MenuPopWidgetState extends State<MenuPopWidget> {
  ...
  Color itemColor = Color.fromRGBO(75, 75, 75, 1.0);
  ...
  void initState() {
    ...
    position = new RelativeRect.fromRect(
      new Rect.fromPoints(
        button.localToGlobal(Offset(-10, 15), ancestor: overlay),
    ...
    return new MediaQuery.removePadding(
      ...
      child: Builder(builder: (BuildContext context) {
        return new CustomSingleChildLayout(
          // 这里计算偏移量
          delegate: new PopupMenuRouteLayout(position, widget.menuHeight + _triangleHeight,
        ...
        child: new Expanded(
          child: Stack(children: <Widget>[
            new ClipRRect(
              ...
              child: Container(color: Color.fromRGBO(255, 255, 255, 1.0), height: widget.menuHeight),
            ),
            new Column(
  ...
  Widget contentBuild() {
    ...
    return SizedBox(
      ...
      width: _curPageWidth,
      child: new Material(color: Colors.transparent, child: body(_curPageWidth)),
    );
  }
  ...
  Widget itemBuild(item) {
    var row = [
      new Padding(
        ...
        child: strNoEmpty(item['icon']) ? new Image.asset(item['icon'],fit: BoxFit.contain,width: 30.0,height: 30.0,) : new Icon(Icons.add_comment, color: Colors.black),
      ),
      new Expanded(
        child: new Container(
          height: 50,
          ...
          decoration: BoxDecoration(
              border: item['title'] == widget.actions[0]['title']
                  ? null
                  : Border(top: BorderSide(color: Colors.black.withOpacity(0.3), width: 0.2))),
          child: new Text(
            item['title'],
